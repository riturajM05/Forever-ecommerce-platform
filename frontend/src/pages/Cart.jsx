          ...
          cartData.map((item, index) => {
            ...
            return (
              <div key={index} className='py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_2fr_0.5fr] items-center gap-4'>
                ...
                <input onChange={(e) => e.target.value === '' || e.target.value === '0' ? null : updateQuantity(updateQuantity(item._id, item.size, Number(e.target.value)))} type="number" min={1} defaultValue={item.quantity} className='border max-h-6 sm:max-h-10 max-w-8 sm:max-w-15 px-1 sm:px-2 py-1' />
